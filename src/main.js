...
const CARDS_COUNT_IN_MAIN_LIST = 5;
const CARDS_COUNT_IN_SUB_LISTS = 2;
...
// Добавляем кнопку 'show more' и карточки фильмов в основной список фильмов
...
for (let i = 0; i < CARDS_COUNT_IN_MAIN_LIST; i++) {
  renderTemplate(mainFilmsListContainer, filmCardTemplate());
}

// Добавляем карточки фильмов в extra список фильмов
const extraFilmsList = filmsListsContainer.querySelectorAll(`.films-list--extra`);

extraFilmsList.forEach((filmsList) => {
  let filmsListContainer = filmsList.querySelector(`.films-list__container`);

  for (let i = 0; i < CARDS_COUNT_IN_SUB_LISTS; i++) {
    renderTemplate(filmsListContainer, filmCardTemplate());
  }
});
